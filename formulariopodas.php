...
<div class="container">
<header>
    <h1 class="">formulario podas</h1>
</header>
    <table  class="table table-striped">
    <tbody>
        <tr>
       
         <th scope="col">tipo poda</th>
         <th scope="col">fecha podas </th>
         <th scope="col">arbol podas </th>
          <th scope="col"></th>
        </tr>
...
$objetopodas = new podas($conexion,0,'idpodas','tipopodas','fechapodas','idarbolpodas','propietario');
$listapodass = $objetopodas->listar($pagina);
$permiso = $objetoarbol->getRol($_SESSION['id']);

if (stripos($permiso,"r")!==false){ 
while($unregistro =mysqli_fetch_array($listapodass)){
    echo '<tr><form id="fmodificarpodas"'.$unregistro["idpodas"].' action="../controlador/controladorpodas.php" method="post">';

    echo '<td><input type="hidden" name="fidpodas"   value="'.$unregistro['idpodas'].'">';
   
    echo ' <input type="text"  class="form-control" name="ftipopodas"  value="'.$unregistro['tipopodas'].'"></td>';
    echo '<td><input type="date" name="ffechapodas"  value="'.$unregistro['fechapodas'].'"></td>';
       echo '</select></td>';

        echo '<td><select name="fidarbolpodas">';
        while($otroregistro =mysqli_fetch_array($listaarboles)){
           echo "<option value=".$otroregistro['idarbol']."";
           if ($unregistro['idarbolpodas'] == $otroregistro['idarbol']){
               echo " selected ";
           }
...
<tr><form id="fingresarpodas" action="../controlador/controladorpodas.php" method="post">
<td><input type="hidden" name="fidpodas" value="0">
<input type="text" class="form-control" name="ftipopodas"></td>
<td><input type="date" name="ffechapodas"></td>
<td><select name="fidarbolpodas">
<?php
       while($otroregistro =mysqli_fetch_array($listaarboles)){
...
<?php
$cantPaginas=$objetopodas->cantPaginas();
if($cantPaginas>1){
    if($pagina>1){
    ...
    for($i=1;$i<=$cantPaginas;$i++){
        if($i==$pagina){
            ...
            echo '<li class="page-item"><a class="page-link" href="formulariopodas.php?pag='.$i.'">'.$i.'</a></li>';
        }
    }
...
<?php
mysqli_free_result($listapodass);
